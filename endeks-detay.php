<?php
/*
  Template Name: Endeks Sayfası
*/
@$endeks = $_GET['e'];
$current_page_id = get_queried_object_id();
global $bp_options;
$borsa = get_data_service("mynet/?url=https://finans.mynet.com/borsa/endeks/$endeks");

preg_match_all('@<h1 class="mr-3">(.*?)</h1@si', $borsa, $endeks_name);
if(empty($endeks_name[1][0]) && empty($_GET['refresh'])){
    // refresh page javascript
    ?><script>
        window.location.href = "<?=get_bloginfo("home")?>/<?=$bp_options['page_endeks']?>/?e=<?=$_GET['e']?>&refresh=true";
    </script><?php
    
}
preg_match_all( '@<div class="data-value"><span class="change-icon  change-(.*?)   mr-2"></span>(.*?)</div><span class="label">Son@si', $borsa, $borsa_value );
preg_match_all( '@<span class="label">Son(.*?)</span>@si', $borsa, $borsa_update );
preg_match_all( '@<div class="heading-new-bar-col-item daily-change"><div class="data-value"><span class="change-icon  change-(.*?)   mr-1"></span>(.*?)</div>@si', $borsa,
	$borsa_rate );
preg_match_all('@<h2>(.*?)</h2>@si', $borsa, $borsa_aname);
$borsa_rate[1][1] = explode(" /", $borsa_rate[2][1]);
$borsa_rate[1][1] = str_replace("( ", null, $borsa_rate[2][0]);
...
// Child theme override: original BirFinans data/parsing logic preserved; only scoped wrapper class is added for Piyasa Vizyon styling.
get_header();
if (empty(@$_GET['e'])) {
    ?><script>
...
<div class="site-wrapper pv-market-native pv-market-index-detail-native">
    ...
    <section class="content home">
        <div class="container-wrap">
                ...
                <div class="singleWrapper">
                    ...
                    <div class="breadcrumb">
                        <ul class="block">
                            ...
                            <li><a href="<?php bloginfo('home') ?>/<?= $bp_options['page_tumendeksler'] ?>/">Tüm Endeksler<i>/</i></a></li>
                    ...
                    <div class="pv-index-detail-content">
                                    ...
                                    <div class="categoryTab">
                                        ...
                                        <div class="catTabContent">
                                            <div class="borsaValue">
                                                ...
                                                <?php if (trim(str_replace(array("%" . ","), array("", "."), $borsa_rate[1][1])) > 0) {
                    ...
                    $crease_color = "#ef291f";
                                                ...
                                                <div class="borsaRate" style="color: <?= $crease_color ?> !important;"><i class="<?= $crease_status ?>"></i>(<?= $borsa_rate[1][1] ?>)</div>
                                            </div>
            ...
            <?php dynamic_sidebar('Sayfa Alt (Endeks Detay)'); ?>
        </div>
    </section>
    <!-- Content -->
    <div class="clear"></div>
