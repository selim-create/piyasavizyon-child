...
<a class="slide<?php echo esc_attr($active); ?>" href="<?php the_permalink(); ?>" <?php if($img) echo 'style="background-image:linear-gradient(to top,rgba(0,0,0,.66),transparent 64%),url('.esc_url($img).');background-size:cover;background-position:center"'; ?>><span class="kicker"><?php echo esc_html($cat ? $cat[0]->name : 'Haber'); ?></span><h2><?php the_title(); ?></h2><p><?php echo esc_html(wp_trim_words(get_the_excerpt(),22)); ?></p></a><?php $active=''; endwhile; wp_reset_postdata(); endif; ?>
  ...
  <?php
  // Sliderlarda seçilmiş içerikleri tekrar etmeden, haber akışını en yeni içerikten başlat.
  $pv_featured_ids = array_values(array_unique(array_merge(
    wp_list_pluck($top->posts, 'ID'),
    wp_list_pluck($s->posts, 'ID')
  )));
  ?>
...
<?php $n=pv_v7_posts(array('posts_per_page'=>7,'post__not_in'=>$pv_featured_ids)); if($n->have_posts()): while($n->have_posts()): $n->the_post(); $img=pv_v7_img(get_the_ID()); ?>
...
<?php $pv_news_ids=wp_list_pluck($n->posts,'ID'); $c=pv_v7_posts(array('posts_per_page'=>12,'post__not_in'=>array_values(array_unique(array_merge($pv_featured_ids,$pv_news_ids))))); if($c->have_posts()): while($c->have_posts()): $c->the_post(); $img=pv_v7_img(get_the_ID()); $cats=get_the_category(); $slugs=array(); if($cats){ foreach($cats as $cat){ $slugs[]=$cat->slug; } } ?><a class="mini-card" data-cats="<?php echo esc_attr(implode(' ', $slugs)); ?>" href="<?php the_permalink(); ?>"><div class="mini-img" <?php if($img) echo 'style="background-image:url('.esc_url($img).');background-size:cover;background-position:center"'; ?>></div><h4><?php the_title(); ?></h4></a><?php endwhile; wp_reset_postdata(); endif; ?>
</div></section>
