<?php get_header(); ?>
<?php
$pv_slider_posts = pv_v7_flagged_posts('bf_anasayfa_slider', array(
  'posts_per_page' => 3,
  'no_found_rows' => true,
));
$pv_kayan_posts = pv_v7_flagged_posts('bf_anasayfa_kayan', array(
  'posts_per_page' => 4,
  'no_found_rows' => true,
));

$pv_excluded_ids = array();
foreach (array($pv_slider_posts, $pv_kayan_posts) as $pv_prefetch_query) {
  if (empty($pv_prefetch_query->posts)) {
    continue;
  }
  foreach ($pv_prefetch_query->posts as $pv_prefetch_post) {
    $pv_excluded_ids[] = (int) $pv_prefetch_post->ID;
  }
}
$pv_excluded_ids = array_values(array_unique($pv_excluded_ids));
?>
...
<?php $top=$pv_slider_posts; if($top->have_posts()): while($top->have_posts()): $top->the_post(); $img=pv_v7_img(get_the_ID(),'medium_large'); ?>
...
<?php
$pv_news_args = array('posts_per_page'=>7);
if (!empty($pv_excluded_ids)) {
  $pv_news_args['post__not_in'] = $pv_excluded_ids;
}
$n=pv_v7_posts($pv_news_args);
if (!empty($n->posts)) {
  foreach ($n->posts as $pv_news_post) {
    $pv_excluded_ids[] = (int) $pv_news_post->ID;
  }
  $pv_excluded_ids = array_values(array_unique($pv_excluded_ids));
}
...
<?php
$pv_agenda_args = array('posts_per_page'=>12);
if (!empty($pv_excluded_ids)) {
  $pv_agenda_args['post__not_in'] = $pv_excluded_ids;
}
